...
class Journal_shapley_value_full_star:
    ...
    def gen_permutation(self,authors_df,current_author, size=0):
        ...
        if size>1:
            ...
            exists=True
            while exists:
                coal_size=random.randint(0,len(authors_df))
                try:
                    if coal_size>0:
                        permutation = list(random.sample(authors, coal_size))
                        while current_author in permutation:
                            permutation = list(random.sample(authors, coal_size))
                    else:
                        permutation=list()
                    permutation.append(current_author)
                    # exists=self.does_permutation_exist(permutation)
                    exists=self.does_coalition_exist(set(permutation))
    ...
    def value_single_permutation_full(self,permutation, authors_df, current_author):
        ...
        for author_id in permutation:
            ...
            total_val += authors_contrib
        if len(papers)>0:
            value_current_coalition = total_val / len(papers)
        else:
            value_current_coalition=0
        author_data = authors_df[authors_df['Author Id'] == current_author]
        authors_contrib = 0
        ...
        for idx, paper_id in enumerate(author_data['papers'].values[0]):
            if not paper_id in papers:
                authors_contrib += author_data['Num citations'].values[0][idx]
                papers.add(paper_id)
        # add current authors contribution to the coalition
        total_val+=authors_contrib
        value_coalition_with_current_author=total_val/len(papers)
        author_marginal_contribution=(value_coalition_with_current_author-value_current_coalition)/num_combinations
        return author_marginal_contribution


    def confidence_shapley_single_author(self,df_authors, current_author, permutation_size):
        print(datetime.now())
        x=0
        k=0
        author_data = authors_df[authors_df['Author Id'] == current_author]
        df_other_authors = authors_df[authors_df['Author Id'] != current_author]

        # citation_to_papers_ratio=author_data['citation_to_papers_ratio'].values[0]
        # if citation_to_papers_ratio==0:
        #     citation_to_papers_ratio+=1
        # min_num_samples=math.log(2/self.delta,base=math.e)/(2*math.pow(self.eps,2))
        min_num_samples=(8*citation_to_papers_ratio*math.log(2/self.delta,math.e))/(math.pow(self.eps,2))
        # min_num_samples=1000
        print('min num samples {}'.format(min_num_samples))
        val = 0
        while k<min_num_samples:
            permutation=self.gen_permutation(df_other_authors,current_author, size=permutation_size)
            ...
            permutation_val=self.value_single_permutation_full(permutation,df_authors,current_author)
            val+=permutation_val
            k+=1
            if k%5000==0:
                print('num samples is {}'.format(k))
        shapley=val/len(authors_df)
...
if __name__ == '__main__':
    dirIS=['Journal of Information Technology',
          'international journal of Law and information Technology',
          'information and organization',
          'ASLIB_Journal_of_info_manage',
          'Communications in Information Literacy',
          'Informing Science',
          'Information Retrieval',
          'Journal of Information Literacy',
          'Library and Information Science Research']
    dirsVirology=['Advances in Virus Research',
                  'The Lancet Microbe', #authors 2791
                  'Current Protocols in Microbiology', #num authors 186, 325 66, num samples 3.8M
                   'Journal of Virus Eradication'
                  ]
    file_path='C:\\Users\\Shir\\OneDrive - Bar Ilan University\\research\\Journals_data\\Virology\\'
    name=dirsVirology[3]
    ...
    js.confidence_shapley(authors_df, column_name='Shapley_star')
    authors_df = authors_df.sort_values(by="Shapley_star", ascending=False)
    print(authors_df)
    # authors_df.to_csv('shap_subsets_10_2.csv')
    authors_df.to_csv(file_path+name+'\\'+'shap_full_star.csv')
